...
async def ai_page(request: Request):
    return templates.TemplateResponse("ai_features.html", {"request": request})

@app.get("/migrate", response_class=JSONResponse)
async def run_migration():
    """Triggers the migration from SQLite to Supabase."""
    try:
        import migrate_to_supabase
        # Force reload if already imported to ensure fresh script state
        import importlib
        importlib.reload(migrate_to_supabase)
        
        result = migrate_to_supabase.migrate()
        if result.get("success"):
            return {"success": True, "message": "Migration completed successfully.", "summary": result.get("summary")}
        else:
...
async def download_db():
    """Notifies that local database is now migrating to Cloud Storage."""
    return JSONResponse(
        status_code=200, 
        content={
            "status": "migrated",
            "message": "Hinata is now using Supabase Cloud! Local DB downloads are disabled for security. Check your Supabase Dashboard for data.",
...
async def control_bot(data: ControlAction):
    if data.action == "restart":
        # Placeholder for restart logic (handled by Render)
        return {"status": "success", "message": "Bot restart initiated via Cloud."}
        ...
        bot.save_config(bot.CONFIG)
        return {"status": "success", "message": f"Global access set to {bot.CONFIG['global_access']}"}
    elif data.action == "ban" and data.target_id:
        success = database.update_user(data.target_id, {"is_banned": True})
        return {"status": "success" if success else "error"}
    elif data.action == "unban" and data.target_id:
        success = database.update_user(data.target_id, {"is_banned": False})
        return {"status": "success" if success else "error"}
    elif data.action == "clear_history" and data.target_id:
        success = database.clear_chat_history(data.target_id)
        return {"status": "success" if success else "error"}
