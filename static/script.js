...
async function migrateData() {
    if (!confirm("Are you sure you want to migrate old SQLite data to Supabase? This should be done only once.")) return;
    showStatus("Migration started...", "info");
    try {
        const res = await fetch('/migrate');
        const data = await res.json();
        if (data.success) {
            const sum = data.summary;
            const msg = `Migration Complete!\nUsers: ${sum.users.success}\nGroups: ${sum.groups.success}\nBroadcasts: ${sum.broadcasts.success}`;
            showStatus(msg, "success");
            refreshData();
        } else {
            showStatus("Migration Failed: " + data.error, "error");
        }
    } catch (e) {
        showStatus("Network error during migration", "error");
    }
}

function showStatus(text, type) {
    alertBox(text, type === 'error' ? 'danger' : type);
    // Visual feedback
    const toast = document.createElement('div');
    toast.className = `v-tag toast-msg ${type}`;
    toast.style.position = 'fixed';
    toast.style.bottom = '20px';
    toast.style.right = '20px';
    toast.style.padding = '10px 20px';
    toast.style.background = type === 'success' ? '#00ff88' : (type === 'error' ? '#ff4b2b' : '#00d2ff');
