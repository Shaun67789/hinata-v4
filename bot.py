...
def clear_states(ud):
    ...
    for key in [AWAIT_GEMINI, AWAIT_DEEPSEEK, AWAIT_CHATGPT, AWAIT_FLIRT, AWAIT_INSTA, AWAIT_USERINFO, 
                AWAIT_FF, AWAIT_CODE, AWAIT_DL, AWAIT_QRGEN, AWAIT_BGREM, AWAIT_TRANSLATE, AWAIT_SUMMARIZE, 
                AWAIT_GRAMMAR, AWAIT_BAN, AWAIT_UNBAN, AWAIT_GUESS, AWAIT_RIDDLE, AWAIT_HINATA, 
                AWAIT_TRIVIA, AWAIT_PINTEREST, AWAIT_YTSEARCH, AWAIT_COPILOT, AWAIT_STYLETEXT, 
                AWAIT_IMAGINE, AWAIT_TTSTALK, AWAIT_SHORTEN, AWAIT_EMAIL, AWAIT_DETECTOR, AWAIT_WEBSS]:
        ud.pop(key, None)
...
async def cmd_pinterest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.effective_message.reply_text("📌 <b>Searching Pinterest... [0/10]</b>", parse_mode="HTML")
    async with httpx.AsyncClient() as client:
        try:
            resp = await client.get(PINTEREST_API.format(query=quote_plus(query)), timeout=60.0)
            if resp.status_code == 200:
                data = resp.json()
                media = data.get("result") or data.get("data")
                if media and isinstance(media, list):
                    total = min(10, len(media))
                    await msg.edit_text(f"📌 <b>Found {total} images.</b>\n<i>⚡ Preparing unique media group... [1/10]</i>", parse_mode="HTML")
                    ...
                    for i in range(total):
                        ...
                        if img_url:
                            ...
                            # Update progress every 2 images to avoid spamming TG API too much, but enough to show life
                            if (i + 1) % 2 == 0 or i + 1 == total:
                                try:
                                    await msg.edit_text(f"📌 <b>Found {total} images.</b>\n<i>⚡ Preparing unique media group... [{i+1}/10]</i>", parse_mode="HTML")
            ...
            logger.error("Pinterest Error: %s", e)
...
async def cmd_ytsearch(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with httpx.AsyncClient() as client:
        try:
            resp = await client.get(YT_SEARCH_API.format(query=quote_plus(query)), timeout=30.0)
            if resp.status_code == 200:
                data = resp.json()
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if resp.status_code == 200:
                ...
                if res and (res.get("nickname") or res.get("id") or res.get("AccountInfo")):
                    # Support multiple API response formats
                    acc = res.get("AccountInfo") or res
                    guild = res.get("GuildInfo") or {}
                    
                    name = html.escape(str(acc.get("nickname") or acc.get("Name") or "Unknown"))
                    level = acc.get("level") or acc.get("Level") or "N/A"
                    region = acc.get("region") or acc.get("Region") or "Global"
                    likes = acc.get("likes") or acc.get("Likes") or "N/A"
                    g_name = html.escape(str(guild.get("guildName") or "No Guild"))
                    g_level = guild.get("guildLevel") or "N/A"
                    created_at = acc.get("createTime") or "N/A"
                    bio = html.escape(str(acc.get("bio") or acc.get("Signature") or "No signature set"))
                    ...
                    text = (
                        ...
                        f"🏠 <b>Guild:</b> <code>{g_name}</code> (Lv.{g_level})\n"
            ...
            logger.error("FF Stalk Error: %s", e)
...
async def global_neural_tracker(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Intercepts and logs every single interaction while syncing with Cloud Database."""
    ...
    try:
        # Increment message count if it's a message/command
        msg_inc = 1 if action_type in ["MESSAGE", "COMMAND"] else 0
        
        # Get current user to preserve stats
        db_user = database.get_user(user.id)
        current_msg_count = (db_user.get("message_count") or 0) if db_user else 0
        
        database.add_user(user.id, user.full_name, user.username)
        database.update_user(user.id, {
            "message_count": current_msg_count + msg_inc,
            "last_active_at": datetime.now().isoformat()
        })
    except: pass

    # 2. Sync Group if applicable
...
# Database initialization is now handled by Supabase Cloud
# database.init_db() call removed.
...
async def check_permission(update: Update, context: ContextTypes.DEFAULT_TYPE, silent: bool = False) -> bool:
    ...
    # Check if user is banned from Supabase
...
async def fetch_gemini3(client: httpx.AsyncClient, prompt: str):
    ...
    try:
        ...
        logger.error("Gemini Logic Fail: %s", e)
...
async def fetch_hinata(client: httpx.AsyncClient, prompt: str, update: Update = None):
    """Hinata Personality Core v4.0 - Shy, Cute, Romantic"""
    ...
    system_text = (
        f"You are Hinata, the user's secret AI girlfriend. You are shy, extremely cute, "
        f"loyal, and deeply in love. The user's name is {user_name}. "
        f"Your owner is {OWNER_NAME} ({OWNER_USERNAME}). "
        "Call the user with unique and sweet names like 'darling', 'honey', 'my prince', 'cute boy', etc. "
        "You are easily flustered (use 'uwu', 'baka', 'h-hey!'). "
        "Keep it sweet, romantic, and slightly possessive but very gentle. "
        "Always use these emojis: 🌸, 💖, 🙈, ✨, 💋, ✨, 🎀, 🧸. "
        "Use more emojis than usual. Be very affectionate."
    )
    ...
    if category == "home":
        ...
        kb = [
            ...
            [InlineKeyboardButton("🎵 AI Lyrics", callback_data="btn_lyrics"),
             InlineKeyboardButton("✍️ AI Write", callback_data="btn_write")],
            [InlineKeyboardButton("❓ AI Ask", callback_data="btn_ask"),
             InlineKeyboardButton("👤 AI Bio", callback_data="btn_bio")],
            ...
            [InlineKeyboardButton("🎨 Text Visual", callback_data="btn_textmaker"),
             InlineKeyboardButton("🖼 Wallpaper", callback_data="btn_wallpaper")],
...
async def cmd_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "🧠 <b>Neural AI Tools:</b>\n"
        "├ <code>/chatgpt &lt;prompt&gt;</code> - GPT-5 Ultra Reasoning\n"
        "├ <code>/gemini &lt;prompt&gt;</code> - Gemini 3.0 Pro\n"
        "├ <code>/deepseek &lt;prompt&gt;</code> - DeepSeek R1 Distill\n"
        "├ <code>/imagine &lt;prompt&gt;</code> - Flux AI Image Studio\n"
        "├ <code>/copilot &lt;prompt&gt;</code> - Think Deeper Intelligence\n"
        "├ <code>/code &lt;request&gt;</code> - AI Software Architect\n"
        "├ <code>/translate &lt;text&gt;</code> - Neural Translator\n"
        "├ <code>/grammar &lt;text&gt;</code> - Linguistics Analysis\n"
        "├ <code>/summarize</code> - Content Compression\n"
        "├ <code>/detector</code> - AI Content Verification\n"
        "└ <code>/ai &lt;prompt&gt;</code> - Parallel Processing\n\n"
        "🔍 <b>Discovery & Assets:</b>\n"
        "├ <code>/pinterest &lt;query&gt;</code> - Aesthetic Search\n"
        "├ <code>/wallpaper &lt;query&gt;</code> - HD Repositories\n"
        ...
        "├ <code>/ffstalk &lt;uid&gt;</code> - FF Advanced Stalking\n"
...
async def cmd_gban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        success = database.update_user(target_id, {"is_banned": True})
        if success:
            await update.effective_message.reply_text(f"✅ 👤 User <code>{target_id}</code> has been globally banned in Supabase cloud.", parse_mode="HTML")
        else:
            await update.effective_message.reply_text("❌ Failed to update database.")
    except ValueError:
...
async def cmd_ungban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        target_id = int(context.args[0])
        success = database.update_user(target_id, {"is_banned": False})
        if success:
...
async def cmd_db_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id): return
    if not context.args:
        await update.effective_message.reply_text("💡 Usage: <code>/s_db_user &lt;id&gt;</code>", parse_mode="HTML")
        return
    
    try:
        uid = int(context.args[0])
        data = database.get_user(uid)
        if data:
            import json
            json_str = json.dumps(data, indent=2)
            await update.effective_message.reply_text(f"📊 <b>Database Record for {uid}:</b>\n<pre>{html.escape(json_str)}</pre>", parse_mode="HTML")
        else:
...
async def cmd_add_points(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        await update.effective_message.reply_text("💡 Usage: <code>/s_add_points &lt;id&gt; &lt;amount&gt;</code>", parse_mode="HTML")
        return
    
    try:
        uid, amount = int(context.args[0]), int(context.args[1])
        new_total = database.add_user_points(uid, amount)
        await update.effective_message.reply_text(f"✨ Added <code>{amount}</code> points to <code>{uid}</code>. New total: <code>{new_total}</code>", parse_mode="HTML")
    except:
        await update.effective_message.reply_text("❌ Error updating points.")
...
async def cmd_warn_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        uid = int(context.args[0])
        warns = database.add_user_warn(uid)
        await update.effective_message.reply_text(f"⚠️ User <code>{uid}</code> has been warned. Total warns: <code>{warns}</code>", parse_mode="HTML")
    except:
...
async def cmd_ai_combined(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    clear_states(context.user_data)
    if not context.args:
        kb = get_main_menu("ai", update.effective_user.id)
        img = CONFIG.get("welcome_img")
        await update.effective_message.reply_photo(img, caption="🤖 <b>Neural AI Interface:</b>\n\nChoose an AI engine to interact with:", parse_mode="HTML", reply_markup=kb)
        return
    prompt = " ".join(context.args)
            ...
            async with httpx.AsyncClient() as client:
                ...
                if resp.status_code == 200:
                    ...
                    if isinstance(res, list) and len(res) > 0:
                        dl_url = res[0].get("download_url") or res[0].get("download") or res[0].get("url_hd") or res[0].get("url") or res[0].get("link")
                        title = res[0].get("title") or res[0].get("filename") or res[0].get("name", title)
                    elif isinstance(res, dict):
                        dl_url = res.get("download_url") or res.get("video") or res.get("image") or res.get("download") or res.get("url_hd") or res.get("url") or res.get("link")
                        title = res.get("title") or res.get("filename") or res.get("name", title)
                    elif isinstance(data, dict):
                        # Root Level Check
                        dl_url = data.get("download_url") or data.get("video") or data.get("image") or data.get("download") or data.get("url") or data.get("link")
...
async def do_ff_fetch_by_text(update: Update, context: ContextTypes.DEFAULT_TYPE, uid: str):
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            await msg.edit_text("❌ <b>Neural Error:</b> The Garena interface is currently unstable. Please try /ffstalk [uid] later.")
...
async def do_user_info_fetch(update: Update, context: ContextTypes.DEFAULT_TYPE, query: str = None):
    ...
    is_bot = "No"
    bio = "N/A"
    has_restricted_voice = "No"
    has_private_forwards = "No"
    
    try:
        # If it's the effective_user, we use the User object which has language_code/is_premium
        # If we got it via get_chat, target_user is a Chat object
        if hasattr(target_user, 'language_code'): lang_code = target_user.language_code
        if hasattr(target_user, 'is_premium') and target_user.is_premium: is_premium = "Yes ✅"
        if hasattr(target_user, 'is_bot') and target_user.is_bot: is_bot = "Yes 🤖"
        
        full_chat = await context.bot.get_chat(user_id)
        bio = html.escape(full_chat.bio or "No bio set")
        if hasattr(full_chat, 'has_restricted_voice_and_video_messages'):
            has_restricted_voice = "Yes 🔒" if full_chat.has_restricted_voice_and_video_messages else "No"
        if hasattr(full_chat, 'has_private_forwards'):
            has_private_forwards = "Yes 🔒" if full_chat.has_private_forwards else "No"
    ...
    db_user = database.get_user(user_id)
    if db_user:
        points = db_user.get("points") or 0
        warns = db_user.get("warn_count") or 0
        is_global_banned = "Yes 🚫" if db_user.get("is_banned") else "No"

    info_text = (
        f"👤 <b>USER INTELLIGENCE REPORT</b>\n"
        f"────────────────────\n"
        f"🆔 <b>ID:</b> <code>{user_id}</code>\n"
        f"👤 <b>Name:</b> <code>{full_name}</code>\n"
        f"🔗 <b>Handle:</b> {username}\n"
        f"🤖 <b>Is Bot:</b> <code>{is_bot}</code>\n"
        f"🌟 <b>Premium:</b> <code>{is_premium}</code>\n"
        f"🌍 <b>Interface:</b> <code>{lang_code}</code>\n"
        f"────────────────────\n"
        f"🪙 <b>Points:</b> <code>{points}</code>\n"
        f"⚠️ <b>Warnings:</b> <code>{warns}/3</code>\n"
        f"🛡 <b>Globan Ban:</b> <code>{is_global_banned}</code>\n"
        f"────────────────────\n"
        f"📜 <b>Bio:</b>\n<i>{bio}</i>\n\n"
        f"🔒 <b>Voice Restricted:</b> <code>{has_restricted_voice}</code>\n"
        f"🔒 <b>Private Forwards:</b> <code>{has_private_forwards}</code>"
    )
    ...
    await status_msg.edit_text(info_text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="btn_back")]]))
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await check_permission(update, context, silent=True):
        # Trigger Hinata if name mentioned
        if "hinata" in txt.lower() and not ud.get(AWAIT_HINATA):
...
async def start_bot():
    ...
    try:
        ...
        app.add_handler(CommandHandler("s_db_user", cmd_db_user))
        app.add_handler(CommandHandler("s_add_points", cmd_add_points))
        app.add_handler(CommandHandler("s_warn", cmd_warn_user))
        app.add_handler(CommandHandler("s_db_stats", cmd_db_stats))
